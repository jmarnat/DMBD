...
box()
legend(x="topright", legend=c("< 5","5 -> 8",">= 8"), cex=0.8,fill=c("#F5BCA9","#F7D358","#D8F781"),bty="n")
...
box()
legend(x="bottomright", legend=c("< 5","5 -> 8",">= 8"), cex=0.8,fill=c("#F5BCA9","#F7D358","#D8F781"),bty="n")
...
barplot(t(cbind(table(data_insatisfied$Meta_donnee.25),table(data_neutral$Meta_donnee.25),table(data_satisfied$Meta_donnee.25))), 
        horiz = TRUE, xlab = "nb of clients", 
        col=c("#F5BCA9","#F7D358","#D8F781"), space = 2, las = 1,
        names.arg=tolower(levels(data_insatisfied$Meta_donnee.25)), 
